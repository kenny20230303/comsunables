...
export default {
    ...
    filteredMaterials() {
      if (this.searchQuery) {
        return this.materials.filter(material =>
          material.name.includes(this.searchQuery) ||
          material.specifications.includes(this.searchQuery) ||
          material.unit.includes(this.searchQuery) ||
          material.user_name.includes(this.searchQuery)
        );
      }
  ...
  methods: {
    fetchMaterials() {
      ...
      this.$axios.post('/auditor/getMonthlyPlanOuter', {user_id})
          .then((res) => {
    ...
    searchMaterials() {
      // This method is triggered on input, but filtering is handled in the computed property
      // You can add any additional logic here if needed
    },

    approveMaterial(row) {
      this.$axios.put('/auditor/getMonthlyPlanOuter', {id: row.id, number: row.Material_number})
          .then((response) => {
